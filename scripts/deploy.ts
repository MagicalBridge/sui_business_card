...
export class ContractDeployer {
  ...
  async deployContract(): Promise<string> {
    try {
      ...
      // 保存包ID和升级权限ID到环境文件
      this.savePackageId(packageId, upgradeCapId);
  ...
  private savePackageId(packageId: string, upgradeCapId?: string): void {
    ...
    // 更新升级权限ID
    if (upgradeCapId && envContent.includes('UPGRADE_CAP_ID=')) {
